...
try{
 Config::assertProductionSafe();$db=Database::connection();$settings=new SettingsService($db);
 if(str_starts_with($path,'/api/v1/otp/'))(new ApiController(new ApiKeyService($db),new RateLimiter($db),new AbuseLimiter($db),$settings,new OtpService($db)))->handle(trim(substr($path,strlen('/api/v1/otp/')),'/'));
 ...
 if(str_starts_with($path,'/api/v1/admin/'))(new AdminController(new AdminService($db),$settings))->handle(trim(substr($path,strlen('/api/v1/admin/')),'/'));
 ...
 if($path==='/verify-email'){$ok=(new CustomerService($db))->verifyEmail((string)($_GET['token']??''));header('Content-Type: text/html; charset=utf-8');echo $ok?'<h1>Email verified</h1><p>Your OTP Auth account is active. You can return to the dashboard.</p>':'<h1>Verification failed</h1><p>The verification link is invalid or expired.</p>';exit;}
