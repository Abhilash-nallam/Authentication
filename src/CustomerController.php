    ...
    public function handle(string $action): never
    {
        ...
        try {
            if($action==='register'){
                ...
                if(!empty($r['verification_token'])){try{$this->mailer->verification($r['email'],$r['verification_token']);}catch(\Throwable $e){Logger::error('customer_verification_email_failed',['error_class'=>get_class($e)]);}}
                Response::success(['message'=>'If this address can be registered, check your email for verification.'],201);
            }
            ...
            if($action==='password-reset-request'){
                $r=$this->customers->requestPasswordReset((string)($data['email']??''));
                if($r){try{$this->mailer->passwordReset($r['email'],$r['token']);}catch(\Throwable $e){Logger::error('customer_password_reset_email_failed',['error_class'=>get_class($e)]);}}
                Response::success(['message'=>'If the account exists, a reset email has been sent.']);
            }
            ...
            if($action==='login'){
                $token=$this->customers->login((string)($data['email']??''),(string)($data['password']??''));$secure=Config::env('APP_ENV','development')==='production';$csrf=bin2hex(random_bytes(24));
                setcookie('otp_auth_session',$token,['expires'=>time()+86400,'path'=>'/','secure'=>$secure,'httponly'=>true,'samesite'=>'Lax']);
                setcookie('otp_auth_csrf',$csrf,['expires'=>time()+86400,'path'=>'/','secure'=>$secure,'httponly'=>false,'samesite'=>'Lax']);Response::success(['message'=>'Login successful.','csrf_token'=>$csrf]);
            }
            if($action==='logout'){
                $token=$this->sessionToken();$this->customers->logout($token);$secure=Config::env('APP_ENV','development')==='production';
                setcookie('otp_auth_session','',['expires'=>time()-3600,'path'=>'/','secure'=>$secure,'httponly'=>true,'samesite'=>'Lax']);setcookie('otp_auth_csrf','',['expires'=>time()-3600,'path'=>'/','secure'=>$secure,'httponly'=>false,'samesite'=>'Lax']);Response::success(['message'=>'Logged out.']);
            }
            $customer=$this->customers->customerFromSession($this->sessionToken());if(!$customer)Response::error('authentication_required','Customer session required.',401);
