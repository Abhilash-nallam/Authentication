    ...
    public function handle(string $action): never
    {
        ...
        try {
            ...
            if ($action==='verify-email') {
                if (!$this->customers->verifyEmail((string)($data['token']??''))) Response::error('invalid_verification_token','Verification token is invalid or expired.',400);
                Response::success(['message'=>'Email verified.']);
            }
            if ($action==='login') {
                $token=$this->customers->login((string)($data['email']??''),(string)($data['password']??''));
                Response::success(['session_token'=>$token]);
            }
            ...
            if ($action==='projects') Response::success(['projects'=>$this->projects->listForCustomer((int)$customer['id'])]);
            if ($action==='project-create') Response::success(['project'=>$this->projects->create((int)$customer['id'],(string)($data['name']??''))],201);
            if ($action==='domain-start') Response::success(['verification'=>$this->domains->start((int)$customer['id'],(int)($data['project_id']??0),(string)($data['domain']??''))]);
            if ($action==='domain-verify') {
                $result=$this->domains->verify((int)$customer['id'],(int)($data['project_id']??0));
                if (!$result['verified']) Response::error('domain_not_verified','DNS TXT record was not found.',400);
                Response::success($result);
            }
            if ($action==='subdomain') Response::success($this->projects->setSubdomain((int)$customer['id'],(int)($data['project_id']??0),(string)($data['subdomain']??'')));
            if ($action==='keys') Response::success(['keys'=>$this->keys->listForProject((int)($data['project_id']??0))]);
            if ($action==='key-create') Response::success(['key'=>$this->keys->create((string)($data['name']??''),(int)($data['project_id']??0))],201);
            if ($action==='key-revoke') { if (!$this->keys->revoke((int)($data['project_id']??0),(int)($data['key_id']??0))) Response::error('key_not_found','API key not found.',404); Response::success(['message'=>'API key revoked.']); }
