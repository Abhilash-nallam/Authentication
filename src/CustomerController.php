...
final class CustomerController
{
    ...
    public function handle(string $action): never
    {
        $data=Request::json();try{
            ...
            if($action==='production-enable')Response::success($this->projects->enableProduction($customerId,(int)($data['project_id']??0)));
            ...
            if($action==='key-create'){if(($data['environment']??'production')==='production'){$p=$this->projects->findOwned($customerId,(int)($data['project_id']??0));if(!$p||$p['status']!=='verified')Response::error('project_not_ready','Verify the project domain before creating production credentials.',409);}$env=(string)($data['environment']??'production');Response::success(['key'=>$this->keys->create((string)($data['name']??''),(int)($data['project_id']??0),$customerId,$env)],201);}
