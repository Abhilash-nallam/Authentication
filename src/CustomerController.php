...
final class CustomerController
{
    ...
    public function handle(string $action): never
    {
        $data=Request::json();
        try {
            if($action==='register'){ $r=$this->customers->register((string)($data['email']??''),(string)($data['password']??'')); try{$this->mailer->verification($r['email'],$r['verification_token']);}catch(\Throwable $e){Logger::error('customer_verification_email_failed',['error_class'=>get_class($e)]);} Response::success(['message'=>'Account created. Check your email to verify it.'],201); }
            ...
            if($action==='logout'){ $token=$this->sessionToken();$this->customers->logout($token);setcookie('otp_auth_session','',['expires'=>time()-3600,'path'=>'/','samesite'=>'Lax']);setcookie('otp_auth_csrf','',['expires'=>time()-3600,'path'=>'/','samesite'=>'Lax']);Response::success(['message'=>'Logged out.']); }
