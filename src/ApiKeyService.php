...
final class ApiKeyService
{
    ...
    {
        $name=trim($name);if($name===''||strlen($name)>120)throw new \InvalidArgumentException('API key name is invalid.');
    ...
    public function rotate(string $name,int $projectId,int $oldKeyId,int $customerId,string $environment='production'): array
    {
        ...
        $this->db->beginTransaction();try{
            $s=$this->db->prepare("UPDATE api_keys SET revoked_at=UTC_TIMESTAMP(),status='revoked',revoked_reason='Rotated' WHERE id=? AND project_id=? AND revoked_at IS NULL AND status='active'");$s->execute([$oldKeyId,$projectId]);
    ...
    private function originAllowed(string $allowed,string $origin): bool{if($allowed==='')return true;if($origin==='')return false;return in_array($origin,$this->listSetting($allowed),true);}
    private function ipAllowed(string $allowed,string $ip): bool
    {
        if($allowed==='')return true;foreach($this->listSetting($allowed) as $rule){if($rule===$ip)return true;if(str_contains($rule,'/')&&$this->cidrMatch($ip,$rule))return true;}return false;
    }
    private function cidrMatch(string $ip,string $cidr): bool
    {
        [$subnet,$bits]=array_pad(explode('/',$cidr,2),2,null);if($bits===null||filter_var($ip,FILTER_VALIDATE_IP)===false||filter_var($subnet,FILTER_VALIDATE_IP)===false)return false;
        $ipBin=inet_pton($ip);$subBin=inet_pton($subnet);if($ipBin===false||$subBin===false||strlen($ipBin)!==strlen($subBin))return false;$bits=(int)$bits;$max=strlen($ipBin)*8;if($bits<0||$bits>$max)return false;
        $bytes=intdiv($bits,8);$rem=$bits%8;if($bytes&&substr($ipBin,0,$bytes)!==substr($subBin,0,$bytes))return false;if($rem===0)return true;$mask=chr((0xFF<<(8-$rem))&0xFF);return (ord($ipBin[$bytes])&ord($mask))===(ord($subBin[$bytes])&ord($mask));
    }
}
