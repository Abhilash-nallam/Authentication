...
final class ApiKeyService
{
    ...
    public function create(string $name, ?int $projectId = null, ?int $customerId = null): array
    {
        ...
        return $this->issue($name,$projectId);
    }

    public function rotate(string $name,int $projectId,int $oldKeyId,int $customerId): array
    {
        if(!$this->ownsProject($customerId,$projectId)) throw new \DomainException('Project not found.');
        $this->db->beginTransaction();
        try {
            $s=$this->db->prepare('UPDATE api_keys SET revoked_at=NOW() WHERE id=? AND project_id=? AND revoked_at IS NULL'); $s->execute([$oldKeyId,$projectId]);
            if($s->rowCount()!==1) throw new \DomainException('Active API key not found.');
            $new=$this->issue($name,$projectId); $this->db->commit(); return $new;
        } catch(\Throwable $e){$this->db->rollBack();throw $e;}
    }

    public function authenticate(string $plain): ?array
    {
        $s=$this->db->prepare('SELECT * FROM api_keys WHERE key_hash=? AND revoked_at IS NULL LIMIT 1');$s->execute([hash('sha256',$plain)]);$row=$s->fetch();
        if(!$row)return null;$this->db->prepare('UPDATE api_keys SET last_used_at=NOW() WHERE id=?')->execute([$row['id']]);return $row;
    }
    ...
    private function issue(string $name,?int $projectId): array
    {
