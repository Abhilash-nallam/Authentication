...
{
    public function __construct(private AdminService $admins) {}
    ...
    public function handle(string $action): never
    {
        ...
        try {
            if ($action==='login') {
                $token=$this->admins->login((string)($data['email']??''),(string)($data['password']??''));
            ...
            $admin=$this->admins->fromSession($this->sessionToken());
            ...
            if ($action==='customer-suspend' || $action==='customer-reactivate') {
                $status=$action==='customer-suspend'?'suspended':'active';
                if (!$this->admins->setCustomerStatus((int)($data['customer_id']??0),$status)) Response::error('customer_not_found','Customer not found.',404);
                Response::success(['message'=>'Customer status updated.','status'=>$status]);
            }
            if ($action==='api-keys') Response::success(['api_keys'=>$this->admins->apiKeys()]);
            if ($action==='key-revoke') {
                if (!$this->admins->revokeKey((int)($data['key_id']??0),(string)($data['reason']??'Revoked by admin'))) Response::error('key_not_found','API key not found.',404);
                Response::success(['message'=>'API key revoked.']);
            }
    ...
    private function permissionFor(string $action): ?string
    {
        return match($action) {
            ...
            'customer-suspend','customer-reactivate' => 'customers.suspend',
            'api-keys' => 'api_keys.revoke',
            'key-revoke' => 'api_keys.revoke',
            'otp-logs' => 'otp_logs.view',
            default => null,
        };
    }
