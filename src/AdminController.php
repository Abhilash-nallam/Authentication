...
final class AdminController
{
    ...
    public function handle(string $action): never
    {
        ...
        try{
            ...
            if($action==='login'){
                $token=$this->admins->login((string)($data['email']??''),(string)($data['password']??''));$csrf=bin2hex(random_bytes(24));
                setcookie('otp_auth_admin',$token,['expires'=>time()+28800,'path'=>'/','secure'=>$secure,'httponly'=>true,'samesite'=>'Strict']);
                setcookie('otp_auth_admin_csrf',$csrf,['expires'=>time()+28800,'path'=>'/','secure'=>$secure,'httponly'=>false,'samesite'=>'Strict']);Response::success(['message'=>'Admin login successful.','csrf_token'=>$csrf]);
            }
            ...
            if($this->usesCookieSession()&&in_array($action,['logout','customer-suspend','customer-reactivate','key-revoke','settings-update'],true)&&!hash_equals((string)($_COOKIE['otp_auth_admin_csrf']??''),(string)($_SERVER['HTTP_X_CSRF_TOKEN']??'')))Response::error('csrf_failed','CSRF token is required.',403);
            if($action==='logout'){$this->admins->logout($this->sessionToken());setcookie('otp_auth_admin','',['expires'=>time()-3600,'path'=>'/','secure'=>$secure,'httponly'=>true,'samesite'=>'Strict']);setcookie('otp_auth_admin_csrf','',['expires'=>time()-3600,'path'=>'/','secure'=>$secure,'httponly'=>false,'samesite'=>'Strict']);Response::success(['message'=>'Logged out.']);}
    ...
    private function usesCookieSession():bool{return !empty($_COOKIE['otp_auth_admin'])&&empty($_SERVER['HTTP_AUTHORIZATION']);}
    private function sessionToken():string{$h=$_SERVER['HTTP_AUTHORIZATION']??'';if(preg_match('/^Bearer\s+(.+)$/i,$h,$m))return trim($m[1]);return(string)($_COOKIE['otp_auth_admin']??'');}
}
