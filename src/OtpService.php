...
final class OtpService
{
    ...
    public function request(string $email, string $purpose, int $apiKeyId, ?int $projectId = null): array
    {
        ...
        $requestId = self::uuid();
        $expiresAt = (new \DateTimeImmutable('now', new \DateTimeZone('UTC')))->modify('+' . Config::int('OTP_TTL_SECONDS', 300) . ' seconds');

        $stmt = $this->db->prepare(
            'INSERT INTO otp_challenges (request_id, api_key_id, project_id, email, purpose, otp_hash, expires_at, max_attempts)
             VALUES (?, ?, ?, ?, ?, ?, ?, ?)'
        );
        $stmt->execute([
            $requestId, $apiKeyId, $projectId, $email, $purpose,
            $this->hashOtp($otp, $email, $requestId),
            $expiresAt->format('Y-m-d H:i:s'), Config::int('OTP_MAX_VERIFY_ATTEMPTS', 5),
        ]);
        ...
        try {
            ...
            $this->db->prepare('DELETE FROM otp_challenges WHERE request_id = ?')->execute([$requestId]);
            $this->recordEmailEvent($projectId, $requestId, 'failed', $email, null, ['error' => $e->getMessage()]);
    ...
    {
        $sql = 'SELECT * FROM otp_challenges WHERE api_key_id = ? AND email = ? AND purpose = ? AND consumed_at IS NULL';
        $params = [$apiKeyId, $email, $purpose];
        if ($requestId !== null) { $sql .= ' AND request_id = ?'; $params[] = $requestId; }
        $sql .= ' ORDER BY created_at DESC LIMIT 1';
        $stmt = $this->db->prepare($sql); $stmt->execute($params); $row = $stmt->fetch();
        if (!$row) return ['verified' => false, 'reason' => 'invalid'];
        if (strtotime($row['expires_at'] . ' UTC') < time()) return ['verified' => false, 'reason' => 'expired'];
        ...
        if (!hash_equals($row['otp_hash'], $expected)) {
            $this->db->prepare('UPDATE otp_challenges SET attempts = attempts + 1 WHERE id = ?')->execute([$row['id']]);
            return ['verified' => false, 'reason' => 'invalid'];
        }
        $this->db->prepare('UPDATE otp_challenges SET consumed_at = NOW() WHERE id = ?')->execute([$row['id']]);
    ...
    {
        $this->db->prepare('UPDATE otp_challenges SET consumed_at = NOW() WHERE api_key_id = ? AND email = ? AND purpose = ? AND consumed_at IS NULL')
            ->execute([$apiKeyId, $email, $purpose]);
    }

    private function hashOtp(string $otp, string $email, string $requestId): string
    {
        return hash_hmac('sha256', $email . '|' . $requestId . '|' . $otp, Config::requireSecret('APP_KEY'));
    }
    ...
    private function sendEmail(string $email, string $otp, string $purpose, \DateTimeImmutable $expiresAt): ?string
    {
        ...
        $result = $this->ses->sendEmail([
            ...
            'Content' => ['Simple' => [
                ...
                'Body' => [
                    'Html' => ['Data' => EmailTemplate::otp($otp, $subject, $expiresAt), 'Charset' => 'UTF-8'],
                    'Text' => ['Data' => "Your OTP is {$otp}. It expires at " . $expiresAt->format('H:i:s') . ' UTC.', 'Charset' => 'UTF-8'],
                ],
            ]],
        ]);
        if ($configurationSet = Config::env('SES_CONFIGURATION_SET')) $result = $result;
        return $result->get('MessageId');
    }

    private function recordEmailEvent(?int $projectId, string $requestId, string $type, string $email, ?string $messageId, array $payload = []): void
    {
        try {
            $this->db->prepare('INSERT INTO email_events (project_id, request_id, event_type, recipient_hash, provider_message_id, payload_json) VALUES (?, ?, ?, ?, ?, ?)')
                ->execute([$projectId, $requestId, $type, hash('sha256', strtolower($email)), $messageId, $payload ? json_encode($payload, JSON_THROW_ON_ERROR) : null]);
        } catch (\Throwable $e) {
            Logger::error('email_event_record_failed', ['request_id' => $requestId]);
        }
    }
