...
final class OtpService
{
    ...
    public function request(string $email, string $purpose, int $apiKeyId): array
    {
        ...
        $stmt->execute([
            $requestId, $apiKeyId, $email, $purpose,
    ...
    public function verify(
        ...
        $sql = 'SELECT * FROM otp_challenges
        ...
        if (!hash_equals($row['otp_hash'], $expected)) {
            $this->db->prepare('UPDATE otp_challenges SET attempts = attempts + 1 WHERE id = ?')
                ->execute([$row['id']]);
            return ['verified' => false, 'reason' => 'invalid'];
        }

        $this->db->prepare('UPDATE otp_challenges SET consumed_at = NOW() WHERE id = ?')
            ->execute([$row['id']]);
