...
final class OtpService
{
    ...
    public function request(string $email,string $purpose,int $apiKeyId,?int $projectId=null): array
    {
        ...
        $stmt=$this->db->prepare('INSERT INTO otp_challenges (request_id,api_key_id,project_id,email,purpose,otp_hash,expires_at,max_attempts) VALUES (?,?,?,?,?,?,?,?)');
        $stmt->execute([$requestId,$apiKeyId,$projectId,$email,$purpose,$this->hashOtp($otp,$email,$requestId),$expiresAt->format('Y-m-d H:i:s'),Config::int('OTP_MAX_VERIFY_ATTEMPTS',5)]);
        try { $messageId=$this->sendEmail($email,$otp,$purpose,$expiresAt); $this->recordEmailEvent($projectId,$requestId,'sent',$email,$messageId); }
        catch(\Throwable $e){ $this->db->prepare('DELETE FROM otp_challenges WHERE request_id=?')->execute([$requestId]); $this->recordEmailEvent($projectId,$requestId,'failed',$email,null,['error_class'=>get_class($e)]); Logger::error('otp_email_failed',['request_id'=>$requestId,'error'=>$e->getMessage()]); throw new \RuntimeException('Unable to deliver OTP email.',0,$e); }
    ...
    public function verify(string $email,string $purpose,string $otp,int $apiKeyId,?string $requestId=null): array
    {
        ...
        if(!$row)return ['verified'=>false,'reason'=>'invalid']; if(strtotime($row['expires_at'].' UTC')<time())return ['verified'=>false,'reason'=>'expired']; if((int)$row['attempts']>=(int)$row['max_attempts'])return ['verified'=>false,'reason'=>'too_many_attempts'];
        if(!hash_equals($row['otp_hash'],$this->hashOtp($otp,$email,$row['request_id']))){$this->db->prepare('UPDATE otp_challenges SET attempts=attempts+1 WHERE id=?')->execute([$row['id']]);return ['verified'=>false,'reason'=>'invalid'];}
        $this->db->prepare('UPDATE otp_challenges SET consumed_at=NOW() WHERE id=?')->execute([$row['id']]); Logger::info('otp_verified',['request_id'=>$row['request_id'],'purpose'=>$purpose]); return ['verified'=>true,'request_id'=>$row['request_id']];
    }

    private function invalidateOpenChallenges(string $email,string $purpose,int $apiKeyId): void{$this->db->prepare('UPDATE otp_challenges SET consumed_at=NOW() WHERE api_key_id=? AND email=? AND purpose=? AND consumed_at IS NULL')->execute([$apiKeyId,$email,$purpose]);}
    ...
    private function sendEmail(string $email,string $otp,string $purpose,\DateTimeImmutable $expiresAt): ?string
    {
        ...
        $result=$this->ses->sendEmail($args); return $result->get('MessageId');
    }
