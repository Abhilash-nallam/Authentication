...
final class AdminService
{
    ...
    public function login(string $email,string $password): string
    {
        ...
        $this->allowAbuse('admin-login-ip:'.Request::ip(),Config::int('ADMIN_LOGIN_MAX_PER_IP_PER_HOUR',20),'Invalid credentials.');
        $this->allowAbuse('admin-login-email:'.hash('sha256',$email),Config::int('ADMIN_LOGIN_MAX_PER_EMAIL_PER_HOUR',10),'Invalid credentials.');
        $stmt=$this->db->prepare('SELECT u.*, r.name AS role_name FROM admin_users u JOIN admin_roles r ON r.id=u.role_id WHERE u.email=? LIMIT 1');$stmt->execute([$email]);$admin=$stmt->fetch();
        if(!$admin||$admin['status']!=='active')throw new \DomainException('Invalid credentials.');
        if(!empty($admin['locked_until'])&&strtotime($admin['locked_until'].' UTC')>time())throw new \DomainException('Invalid credentials.');
        if(!password_verify($password,$admin['password_hash'])){
            $attempts=(int)$admin['failed_attempts']+1;$locked=$attempts>=5?gmdate('Y-m-d H:i:s',time()+900):null;
            $this->db->prepare('UPDATE admin_users SET failed_attempts=?,locked_until=? WHERE id=?')->execute([$attempts,$locked,$admin['id']]);throw new \DomainException('Invalid credentials.');
        }
        ...
        $token=bin2hex(random_bytes(32));$this->db->prepare('INSERT INTO admin_sessions(admin_id,token_hash,expires_at) VALUES(?,?,?)')->execute([$admin['id'],hash('sha256',$token),gmdate('Y-m-d H:i:s',time()+28800)]);return $token;
    }
